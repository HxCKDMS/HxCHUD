...
public class RenderXP {
    ...
    public void renderXPBar(RenderGameOverlayEvent.Pre event) {
        if (event.getType() == RenderGameOverlayEvent.ElementType.EXPERIENCE && Configuration.useCustomPosition) {
            ...
            if (xpLast != mc.player.experienceTotal) {
            ...
            if (i > 0) {
                int k = (int)(mc.player.experience * bar.textureSizeX);
            ...
            if (mc.player.experienceLevel > 0)
            {
                ...
                int i1 = (scaledresolution.getScaledWidth() - mc.fontRenderer.getStringWidth(s)) / 2;
                int j1 = scaledresolution.getScaledHeight() - 31 - 4;
                mc.fontRenderer.drawString(s, i1 + 1, j1, 0);
                mc.fontRenderer.drawString(s, i1 - 1, j1, 0);
                mc.fontRenderer.drawString(s, i1, j1 + 1, 0);
                mc.fontRenderer.drawString(s, i1, j1 - 1, 0);
                mc.fontRenderer.drawString(s, i1, j1, 8453920);
                mc.mcProfiler.endSection();
            }

            xpLast = mc.player.experienceTotal;
