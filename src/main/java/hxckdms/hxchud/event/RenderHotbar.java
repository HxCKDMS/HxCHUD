...
public class RenderHotbar {
        ...
        if (event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR && Configuration.useCustomPosition) {
            ...
            if (bar.uiPosX == -9999 && bar.uiPosY == -9999) {
                ...
                if (bar.hideWhenUnchanged)
                    shiftedPos = getCalculatedHiddenPos(bar, (lastItem == null && player.getHeldItemMainhand() != null) || (lastItem != null && player.getHeldItemMainhand() == null) || (lastItemSlot != player.inventory.currentItem || lastItem != player.getHeldItemMainhand().getItem()));
                ...
                for (int slot = 0; slot < 9; ++slot) {
                    renderHotbarItem(scaledWidth - 91 + 3 + shiftedPos[0] + (slot * 20), scaledHeight - 22 + shiftedPos[1], event.getPartialTicks(), player, player.inventory.mainInventory[slot]);
                }
            } else {
                int[] shiftedPos = new int[]{0,0};
                if (bar.hideWhenUnchanged)
                ...
                if (bar.subElements.get("Slot1Pos").get(0) < -1) {
                    if (!bar.verticle) {
                        for (int x = 0; x < 9; x++) {
                            renderHotbarItem(4 + startX + (x * bar.textureSizeX - 1) + shiftedPos[0], startY + 3 + shiftedPos[1], event.getPartialTicks(), player, player.inventory.mainInventory[x]);
                        }
                    } else {
                        for (int y = 0; y < 9; y++) {
                            renderHotbarItem(4 + startX + shiftedPos[0], startY + (y * bar.textureSizeY) + shiftedPos[1] + 3, event.getPartialTicks(), player, player.inventory.mainInventory[y]);
                        }
                    }
                } else {
                    for (int slot = 0; slot < 9; slot++) {
                        renderHotbarItem(4 + bar.subElements.get("Slot" + (slot + 1) + "Pos").get(0), bar.subElements.get("Slot" + (slot + 1) + "Pos").get(1) + 3, event.getPartialTicks(), player, player.inventory.mainInventory[slot]);
                    }
                }
            }
    ...
    public void renderHotbarItem(int x, int y, float partialTicks, EntityPlayer player, ItemStack stack) {
        ...
        if (stack != null) {
            ...
            itemRenderer.renderItemOverlays(this.mc.fontRendererObj, stack, x, y);
        }
    }
